    ...
    switch (event) {
		...
		case ESP_GAP_BLE_SCAN_RESULT_EVT:
			...
			if(param->scan_rst.search_evt == ESP_GAP_SEARCH_INQ_RES_EVT) {
				...
				//if (!(param->scan_rst.bda[0]==0x1d || param->scan_rst.bda[0]==0x5d || param->scan_rst.bda[0]==0x3d || param->scan_rst.bda[0]==0x2a || param->scan_rst.bda[0]==0x79 || param->scan_rst.bda[0]==0x2e || param->scan_rst.bda[0]==0x1c || param->scan_rst.bda[0]==0x67  || param->scan_rst.bda[0]==0x4b )){
				// a device was found, check if it's an iBeacon
				if (1) /*!((param->scan_rst.bda[0]==0x9c)||(param->scan_rst.bda[0]==0xe0))*/ {
					printf("something has been detected!\r\n ");
					printf("Device found: ADDR=");
					for(int i = 0; i < ESP_BD_ADDR_LEN; i++) {
						printf("%02X", param->scan_rst.bda[i]);
					...
					for (int lm=0;lm<sizeof(param->scan_rst.ble_adv);lm++){
						printf("%x:",param->scan_rst.ble_adv[lm]);
					}
					printf("\r\n");
				}
				// first verify that data length is 30 bytes
				if ((param->scan_rst.ble_adv != NULL) && (param->scan_rst.adv_data_len == 0x1E)) {
					for (int l=0;l<sizeof(param->scan_rst.ble_adv);l++){
						printf("%d",param->scan_rst.ble_adv[l]);
					}
